...
def laserCallback(scan):
    ...
    for i in range(0, len(scan.ranges)):
        ...
        final_x = (origin_offset[0] + (point[0]*100) + robot_pose[0])*X_SCALE
        final_y = (origin_offset[1] - robot_pose[1] - (point[1]*100))*Y_SCALE
...
def obstacleCallback(data):
    ...
    #    pygame.draw.circle(screen, YELLOW, (int(obs_x), int(obs_y)), 3)
    #refresh_screen()
